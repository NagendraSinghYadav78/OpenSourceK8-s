<#
.SYNOPSIS
    RESOURCE-MATCHED rerun of the K3 (Rancher Desktop) deployment study.
    Identical experimental logic to k3_experiment_protocol.ps1, but run
    AFTER capping Rancher Desktop's WSL2 environment to 2 CPU / 5 GB via
    .wslconfig, to match Minikube's explicit 2 CPU / 5000 MB allocation.
    Writes to .\results-matched\ so the original (unmatched) results in
...
.NOTES
    Before running this script:
        1. Create/edit %UserProfile%\.wslconfig with:
               ...
               memory=5GB
        2. Run: wsl --shutdown
        3. Restart Rancher Desktop and wait for Kubernetes to come back up
        4. Verify with: wsl -d rancher-desktop nproc
                         wsl -d rancher-desktop free -h
           (should now show 2 processors and ~5GB memory)
...
Write-Host "IMPORTANT: Confirm the resource cap was actually in effect during this run:"
Write-Host "  wsl -d rancher-desktop nproc      (should show 2)"
Write-Host "  wsl -d rancher-desktop free -h    (should show ~5 GB total)"
